...
import { triggerDealFlowAlerts } from '@/lib/investor/deal-flow-alerts'
...
export async function POST(_req: NextRequest) {
  try {
    ...
    // 3. Build section map + merge to AssessmentData
    ...
    if (!process.env.VOYAGE_API_KEY) {
      log.warn('[profile-builder/submit] VOYAGE_API_KEY not configured — founder embedding skipped', { userId })
    }
    if (process.env.VOYAGE_API_KEY) {
      void (async () => {
        try {
