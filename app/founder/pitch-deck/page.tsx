...
import { usePitchDeckData } from "@/features/founder/hooks/usePitchDeckData";
...
export default function PitchDeckPage() {
  const { data: deckData, loading, refreshing, refresh } = usePitchDeckData();
  const [activeSlide, setActiveSlide] = useState(0);
  const [companyName, setCompanyName] = useState("Your Company");

  const slides       = deckData ? buildSlides(deckData.artifacts) : [];
  const artifactCount = deckData?.artifactCount ?? 0;

  useEffect(() => {
    if (deckData?.companyName && deckData.companyName !== "Your Company") {
      setCompanyName(deckData.companyName);
    }
  }, [deckData]);
  ...
  return (
    <div style={{ background: bg, minHeight: "100vh", padding: "32px 40px" }}>
      ...
      <div style={{ background: surf, borderRadius: 12, padding: "14px 18px", border: `1px solid ${bdr}`, marginBottom: 28, display: "flex", gap: 20 }}>
        ...
        {missingCount > 0 && (
          <Link href="/founder/executive" style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 4, fontSize: 12, color: blue, fontWeight: 600, textDecoration: "none" }}>
            ...
            >
              ...
              <p style={{ fontSize: 13, color: "#92400E", flex: 1 }}>
                ...
                {current.sourceArtifacts.map((a, i) => (
                  <span key={a}>
                    <Link href={`/founder/executive`} style={{ color: blue, fontWeight: 600, textDecoration: "none" }}>
                      {a.replace(/_/g, " ")}
                    </Link>
