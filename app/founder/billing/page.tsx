    ...
    <div style={{ minHeight: '100vh', background: bg, color: ink, padding: '48px 24px 100px', fontFamily: 'system-ui,-apple-system,sans-serif' }}>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 48 }}>
          ...
          <p style={{ fontSize: 14, color: muted, marginTop: 6 }}>Manage your plan, usage, and billing history.</p>
        </div>

        {/* Plan row */}
        <div style={{ padding: '20px 0', borderTop: `1px solid ${bdr}`, borderBottom: `1px solid ${bdr}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 40 }}>
          {loading ? (
            ...
            <>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 12 }}>
                ...
                {isPremium && billing?.periodEnd && (
                  <span style={{ fontSize: 13, color: muted }}>Renews {fmt(billing.periodEnd)}</span>
                )}
                {!isPremium && (
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                {billing?.subscriptionStatus && billing.subscriptionStatus !== 'inactive' && (
                  <span style={{ fontSize: 12, color: billing.subscriptionStatus === 'active' ? green : amber }}>
                    {billing.subscriptionStatus === 'active' ? 'Active' : billing.subscriptionStatus}
                  </span>
                )}
                {isPremium && (
                  <button onClick={handleManage} disabled={acting} style={{ padding: '7px 14px', borderRadius: 7, border: `1px solid ${bdr}`, background: 'transparent', color: ink, fontSize: 12, fontWeight: 500, cursor: acting ? 'not-allowed' : 'pointer', opacity: acting ? 0.5 : 1 }}>
        ...
        {u && (
          <div style={{ marginBottom: 48 }}>
            <p style={{ fontSize: 12, fontWeight: 600, color: muted, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 20 }}>Usage this month</p>
        ...
        {isPremium && !loading && (
          <div style={{ marginBottom: 48, padding: '20px 0', borderTop: `1px solid ${bdr}` }}>
            <p style={{ fontSize: 12, fontWeight: 600, color: muted, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 16 }}>Included features</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {FEATURES.map(([label]) => (
                <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <Check style={{ height: 11, width: 11, color: green, flexShrink: 0 }} />
        ...
        <div style={{ borderTop: `1px solid ${bdr}` }}>
          ...
          >
            ...
            {invoicesLoading
              ? <RefreshCw style={{ height: 12, width: 12, color: muted, animation: 'spin 1s linear infinite' }} />
              : <ChevronDown style={{ height: 12, width: 12, color: muted, transform: historyOpen ? 'rotate(180deg)' : 'none', transition: 'transform .2s' }} />
            }
          </button>
          ...
          {historyOpen && invoicesLoaded && (
            <div style={{ paddingBottom: 24 }}>
              {invoices.length === 0 ? (
                <p style={{ fontSize: 13, color: muted, padding: '8px 0' }}>No invoices yet.</p>
              ) : invoices.map((inv, i) => {
                ...
                return (
                  <div key={inv.id} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '12px 0', borderTop: i > 0 ? `1px solid ${bdr}` : 'none' }}>
                    <div style={{ flex: 1 }}>
                      <p style={{ fontSize: 13, color: ink, margin: 0 }}>{inv.description}</p>
                      <p style={{ fontSize: 11, color: muted, margin: '2px 0 0' }}>{date}</p>
                    </div>
                    ...
                    {inv.pdfUrl && (
                      <a href={inv.pdfUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 4, color: muted, fontSize: 11, textDecoration: 'none' }}>
