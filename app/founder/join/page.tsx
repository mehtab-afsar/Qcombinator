...
const pills = [
  ...
  { icon: Users,      text: "5 AI founder executives"  },
  ...
  return (
    <div style={{ minHeight: "100vh", background: bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px 16px" }}>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
        style={{ width: "100%", maxWidth: 460, background: "white", border: `1px solid ${bdr}`, borderRadius: 20, overflow: "hidden", boxShadow: "0 8px 40px rgba(0,0,0,0.06)" }}
      >
        {loading ? (
          <div style={{ padding: "64px 0", textAlign: "center" }}>
            <Loader2 style={{ width: 28, height: 28, color: muted, margin: "0 auto 12px" }} className="animate-spin" />
            <p style={{ fontSize: 14, color: muted }}>Validating your invite…</p>
          </div>
          ...
          <div style={{ padding: "48px 32px", textAlign: "center" }}>
            ...
            <Link href="/" style={{ fontSize: 13, color: blue, textDecoration: "none" }}>Go to homepage →</Link>
          </div>

        ) : info?.type === "team" ? (
          /* ── Team invite card ─────────────────────────────────────────── */
          <>
            <div style={{ padding: "28px 32px 24px", borderBottom: `1px solid ${bdr}`, background: `${purple}08` }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
                <div style={{ width: 44, height: 44, borderRadius: 12, background: `${purple}15`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <UserPlus style={{ width: 20, height: 20, color: purple }} />
                </div>
                <div>
                  <p style={{ fontSize: 12, color: muted, marginBottom: 2 }}>Team invite from</p>
                  <p style={{ fontSize: 16, fontWeight: 700, color: ink }}>{info.inviterName}</p>
                </div>
              </div>
              <p style={{ fontSize: 14, color: ink, lineHeight: 1.6, marginBottom: 12 }}>
              ...
              <div style={{ display: "inline-block", padding: "3px 10px", borderRadius: 999, background: `${purple}15`, fontSize: 11, fontWeight: 700, color: purple, letterSpacing: "0.06em" }}>
                {ROLE_LABELS[info.role] ?? info.role}
              </div>
            </div>

            <div style={{ padding: "20px 32px" }}>
              <p style={{ fontSize: 12, fontWeight: 700, color: muted, letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 14 }}>You&apos;ll get access to</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 24 }}>
                {pills.map(p => (
                  <div key={p.text} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 28, height: 28, borderRadius: 7, background: `${purple}10`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <p.icon style={{ width: 13, height: 13, color: purple }} />
                    </div>
                    <p style={{ fontSize: 13, color: ink }}>{p.text}</p>
                  </div>
                ))}
              </div>
              ...
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                ...
                disabled={joining}
                style={{ width: "100%", padding: "13px 0", borderRadius: 10, border: "none", background: joining ? muted : purple, color: "white", fontSize: 15, fontWeight: 700, cursor: joining ? "default" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, boxShadow: "0 4px 16px rgba(124,58,237,0.3)" }}
              >
            ...
            <div style={{ borderTop: `1px solid ${bdr}`, padding: "12px 32px", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
              <CheckCircle style={{ width: 13, height: 13, color: green }} />
              <span style={{ fontSize: 11, color: muted }}>Invite verified · Shared workspace</span>
            </div>
          </>
          ...
          <>
            <div style={{ padding: "28px 32px 24px", borderBottom: `1px solid ${bdr}`, background: surf }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
                <div style={{ width: 44, height: 44, borderRadius: 12, background: `${blue}15`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <Building2 style={{ width: 20, height: 20, color: blue }} />
                </div>
                <div>
                  <p style={{ fontSize: 12, color: muted, marginBottom: 2 }}>Invite from</p>
                  <p style={{ fontSize: 16, fontWeight: 700, color: ink }}>
                    {info.investorName ?? "Your investor"}
                    {info.firmName && <span style={{ fontWeight: 400, color: muted }}> · {info.firmName}</span>}
                  </p>
                </div>
              </div>
            ...
            <div style={{ padding: "20px 32px" }}>
              <p style={{ fontSize: 12, fontWeight: 700, color: muted, letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 14 }}>What you&apos;ll get</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 24 }}>
                {pills.map(p => (
                  <div key={p.text} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 28, height: 28, borderRadius: 7, background: `${blue}10`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <p.icon style={{ width: 13, height: 13, color: blue }} />
                    </div>
                    <p style={{ fontSize: 13, color: ink }}>{p.text}</p>
                  </div>
                ))}
              ...
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                ...
                style={{ width: "100%", padding: "13px 0", borderRadius: 10, border: "none", background: blue, color: "white", fontSize: 15, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, boxShadow: "0 4px 16px rgba(37,99,235,0.3)" }}
              >
                Create your account <ArrowRight style={{ width: 16, height: 16 }} />
              </motion.button>
              <p style={{ fontSize: 11, color: muted, textAlign: "center", marginTop: 10 }}>
                Already have an account?{" "}
                <a href="/login" style={{ color: blue, textDecoration: "none" }}>Sign in instead</a>
              </p>
            </div>

            <div style={{ borderTop: `1px solid ${bdr}`, padding: "12px 32px", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
              <CheckCircle style={{ width: 13, height: 13, color: green }} />
              <span style={{ fontSize: 11, color: muted }}>Invite verified · Free to join</span>
            </div>
          </>
        ) : null}
      </motion.div>
      ...
      <p style={{ marginTop: 24, fontSize: 12, color: muted }}>
        © {new Date().getFullYear()} Edge Alpha · <a href="/privacy" style={{ color: muted }}>Privacy</a>
      </p>
    </div>
  );
}
