...
**Cost:** v1 runs **sequentially** (parallelism where dependencies allow is a deferred optimisation — correctness first) and omits the Q-Score from the compose context (additive, noted in the F10 design note (removed 4 Aug 2026 — in git history)).
...
exit criterion was unmeetable and week-4 retention indefensible (see the B8 decision paper (removed 4 Aug 2026 — its outcome is ADR-028 below)). The
...
**Why:** "frozen" means *stop adding capability*, not *preserve a known security hole for months*. Five of the six `CRON_SECRET` consumers already fail closed — this was an oversight, not a design choice. See the Phase 0 audit (removed 4 Aug 2026 — in git history) §2, §7a.
