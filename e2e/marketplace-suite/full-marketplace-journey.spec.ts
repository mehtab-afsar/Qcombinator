...
test.describe.serial('Full Marketplace Journey', () => {
  ...
  test('Phase 2 — Patel D1→D4→GTM Playbook, Q-Score strictly increases', async () => {
    ...
    await run('Patel D4: Positioning & Messaging generated', async () => {
      await chatWithAgent(founderPage, 'patel', BUILD_MESSAGES.d4, { timeout: 120_000 })
      await waitForArtifactCard(founderPage, 'positioning_messaging', 120_000)
    ...
    // Reload again before GTM Playbook (heaviest deliverable)
    await founderPage.goto('/founder/cxo/patel')
    await founderPage.waitForLoadState('networkidle')

    // GTM Playbook
    await run('Patel: GTM Playbook generated', async () => {
      await chatWithAgent(founderPage, 'patel', BUILD_MESSAGES.gtm, { timeout: 150_000 })
      await waitForArtifactCard(founderPage, 'gtm_playbook', 150_000)
