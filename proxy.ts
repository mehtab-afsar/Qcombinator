...
export async function proxy(request: NextRequest) {
  ...
  // Refresh the auth token — wrapped in try/catch so a down Supabase
  // instance (e.g. local Docker not started) doesn't crash every request.
  let user = null
  try {
    const { data } = await supabase.auth.getUser()
    user = data.user
  } catch {
    // Supabase unreachable (local dev without Docker, or network issue)
    // Treat as unauthenticated — protected routes will redirect to onboarding
  }
  ...
  if (request.nextUrl.pathname.startsWith('/founder') && !isPublicFounderRoute) {
    ...
    if (request.nextUrl.pathname === '/founder/matching') {
      try {
        ...
        // DB unreachable — don't block the route
      }
    }
  }
