...
describe('the OAuth callback route recognizes investor intent for a brand-new sign-up', () => {
  ...
  it('sends a brand-new investor to /investor/onboarding, not /founder/onboarding', () => {
    // Bounded by the founder REDIRECT, not the founder lookup. The lookup is a side-effect-free
    // SELECT and has since moved above the pre-launch gate (which must see both profiles before
    // deciding whether someone is new); where it sits changes nothing. The redirect is what
    // actually decides where a person lands, so that is what these assertions bound.
    const block = src.slice(src.indexOf("intent === 'investor'"), src.indexOf('if (founderProfile) {'))
  ...
  it('outranks the founder redirect — dual-role is supported', () => {
    // An existing investor_profiles row still wins outright (they're already onboarded on that
    // side — go straight to the dashboard). But intent must outrank the FOUNDER REDIRECT:
    // dual-role is supported, so an existing founder explicitly signalling investor intent must
    // not be shunted into founder onboarding just because that row happens to exist.
    //
    // Previously this asserted on the position of the founder LOOKUP. That was a proxy, and it
    // broke when the lookup moved above the pre-launch gate — which needs to see both profiles
    // before it can tell a new person from a returning one. The lookup has no side effects; the
    // redirect is the behaviour.
    const intentIdx = src.indexOf("intent === 'investor'")
    const investorCheckIdx = src.indexOf("from('investor_profiles')")
    const founderRedirectIdx = src.indexOf('if (founderProfile) {')
    expect(investorCheckIdx).toBeGreaterThan(-1)
    expect(investorCheckIdx).toBeLessThan(intentIdx)
    expect(founderRedirectIdx).toBeGreaterThan(-1)
    expect(founderRedirectIdx).toBeGreaterThan(intentIdx)
  })
